...
/usr/bin/gps_get_coords.sh
lat=$(cat /tmp/gpslat)
lon=$(cat /tmp/gpslon)
(>&2 echo $lat, $lon)
key=$(cat /usr/share/rfsom-box-gui/gmaps.key)
if [ "$key" = "paste-google-api-key-here" ];
then
(>&2 echo Put google api key in /usr/share/rfsom-box-gui/gmaps.key)
exit 1
fi;
...
/usr/bin/fim --device /dev/fb0 -a -q /tmp/gmap.png 2>/dev/null
